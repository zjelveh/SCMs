...
outcomeGet <- function(Y.pre.fit, Y.post.fit, Y.df, units.est, treated.units,
  ...
  if (plot.type == "time") {
    ...
    if (methods::is(res.df[["Time"]], "Date")) {
      # flag that will be used later to handle synth labels
      timeVarIsDate <- TRUE
    } else {
      timeVarIsDate <- FALSE
    }
    ...
    if (timeVarIsDate == TRUE) {
      # first convert from string to date and then numeric
