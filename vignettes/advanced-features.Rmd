...
process_large_spec_curve <- function(dataset, max_specs_per_batch = 50) {
  ...
  covariate_specs <- list(
    basic = list(
      label = "basic",
      operations = list(
        list(var = "covar_1", compute = "mean"),
        list(var = "covar_2", compute = "mean")
      )
    ),
    extended = list(
      label = "extended",
      operations = list(
        list(var = "covar_1", compute = "mean"),
        list(var = "covar_2", compute = "mean"),
        list(var = "covar_3", compute = "mean"),
        list(var = "covar_4", compute = "mean")
      )
    ),
    behavioral = list(
      label = "behavioral",
      operations = list(
        list(var = "covar_5", compute = "mean"),
        list(var = "covar_6", compute = "mean")
      )
    )
  )
  ...
  analysis_params <- list(
    ...
    inference_config = list(verbose = TRUE)
  )

  # Process analysis through the wrapper
...
```{r xgboost-integration, eval=FALSE}
# Advanced SHAP analysis with XGBoost
run_advanced_shap_analysis <- function(spec_results, xgboost_config = NULL) {

  if (is.null(xgboost_config)) {
  ...
  shap_results <- run_xgboost_shap_analysis(spec_results, xgboost_config)
